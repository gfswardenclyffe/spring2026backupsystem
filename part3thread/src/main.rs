...
impl Worker {
    ...
    fn new(id: usize, receiver: Arc<Mutex<mpsc::Receiver<Message>>>) -> Worker {
        ...
        let thread = thread::spawn(move || {
            loop {
                /// Lock the shared receiver (so only one thread accesses it at a time)
        // .recv() blocks (waits) until a message arrives
        // unwrap() is used to extract the value or crash if something goes wrong
                let message = receiver.lock().unwrap().recv().unwrap();

                match message {
                    Message::NewJob(job) => {//Case 1
                        println!("Worker {} got a job; executing.", id);
                        job();
                    }
                    Message::Terminate => {//Case 2
